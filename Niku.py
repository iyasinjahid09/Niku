    ...
    if expiration_date:
        try:
            ...
            if current_time < exp_datetime:
                ...
                expiry_text += f"🕒 𝗥𝗲𝗺𝗮𝗶𝗻𝗶𝗻𝗴: {days}d {hours}h {minutes}min\n"
            else:
                status = "Inactive ❌"
...
def handle_admin_actions(message):
    ...
    for user_id, expiry in users.items():
        try:
            ...
            exp_datetime = datetime.datetime.strptime(expiry, '%Y-%m-%d %H:%M:%S')
            current_time = datetime.datetime.now()

            # Calculate remaining time if access is active
            if current_time < exp_datetime:
                remaining_time = exp_datetime - current_time
                days = remaining_time.days
                hours, remainder = divmod(remaining_time.seconds, 3600)
                minutes, _ = divmod(remainder, 60)
                remaining_text = f"🕒 𝗥𝗲𝗺𝗮𝗶𝗻𝗶𝗻𝗴: {days}d {hours}h {minutes}min"
            else:
                remaining_text = "🚫 𝗘𝘅𝗽𝗶𝗿𝗲𝗱"

        except Exception as e:
            username = "❗️ 𝗨𝘀𝗲𝗿 𝗡𝗼𝘁 𝗙𝗼𝘂𝗻𝗱"
            remaining_text = "🚫 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗱𝗮𝘁𝗲"

        # Add user details to the list
        user_list += (f"👤 𝗨𝘀𝗲𝗿𝗻𝗮𝗺𝗲: {username}\n"
                      f"🆔 𝗨𝘀𝗲𝗿 𝗜𝗗: `{user_id}`\n"
                      f"🛅 𝗘𝘅𝗽𝗶𝗿𝗮𝘁𝗶𝗼𝗻: `{convert_utc_to_ist(expiry)}`\n"
                      f"{remaining_text}\n\n")
